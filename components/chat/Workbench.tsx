...
export function Workbench({
  ...
  const isCurrentFileUnsaved = currentDocument
  ...
  const handleSave = useCallback(async () => {
    if (currentDocument) {
      ...
      if (success) {
        toast({
          title: "File Saved",
          description: `${currentDocument.filePath.split("/").pop()} saved.`,
        });

        // Trigger sync after save
        if (webContainerInstance && projectId && session?.user?.id) {
          try {
            const files = await getAllFilesFromWebContainer(
              webContainerInstance
            );
            await fetch(`/api/projects/${projectId}/sync`, {
              method: "POST",
