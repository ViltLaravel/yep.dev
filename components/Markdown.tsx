  ...
  ({
    ...
    return (
      <div className="relative group my-3 rounded-lg overflow-hidden bg-[#161618] shadow-sm border border-[#313133]">
        ...
        <div
          className={cn(
            "text-sm overflow-x-auto break-words whitespace-pre-wrap",
            isBash ? "p-3" : "p-4"
          )}
        >
          <SyntaxHighlighter
...
const FileUpdatesPanel = memo(
  ({
    ...
    return (
      <div className="mb-4 border-none overflow-hidden rounded-lg w-full shadow-sm">
        <div className="p-3 bg-slate-50 text-gray-600 font-medium border-">
        ...
        <div className="p-3 bg-slate-50 space-y-2">
          {completedFilesArray.map((filePath) => (
            <div
              key={`file-${filePath}`}
              className="flex items-center text-sm bg-slate-200 py-1.5 px-3 text-gray-600 rounded-md border border-slate-100"
            >
              ...
              <span className="text-gray-600 break-all">
                ...
                <button
                  onClick={() => {
                    import("@/app/lib/stores/workbenchStore").then(
                      ({ setSelectedFile, setWorkbenchView }) => {
...
const safePrepareBoltContentForStreaming = (
  content: string,
  isStreaming: boolean
) => {
  if (!isStreaming) {
  ...
  if (
    cleanedContent.includes("<boltArtifact") ||
    cleanedContent.includes("<boltAction")
  ) {
  ...
  if (matches.length > 0) {
    processedContent = cleanedContent.replace(
      completeActionRegex,
      (match, filePath, code) => {
  ...
  const completeShellRegex =
    /<boltAction\s+type="(command|shell)"[^>]*>([\s\S]*?)<\/boltAction>/g;
...
export const Markdown = memo(
  ({
    ...
    const { artifactTitle, cleanedContent, hasBoltActionTags } =
      processContent(content);

    const displayContent = safePrepareBoltContentForStreaming(
      cleanedContent,
      isStreaming || false
    );
    ...
    return (
      <div
        ref={containerRef}
        className="markdown-content prose prose-invert prose-sm max-w-none text-gray-500 break-words overflow-hidden w-full"
